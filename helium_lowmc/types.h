...
template <typename GF, typename GF_CHECK> struct repetition_proof_t {
  reveal_list_t reveallist;
  std::vector<uint8_t> Com_e;
  std::vector<uint8_t> sk_delta;
  std::vector<GF> z_delta;
  std::vector<GF> p_delta;
  GF_CHECK missing_alpha;
  GF_CHECK c_delta;
};

template <typename GF, typename GF_CHECK> struct signature_t {
  salt_t salt;
  std::vector<uint8_t> h_1;
  // std::vector<uint8_t> h_2; // this gets recomputed
