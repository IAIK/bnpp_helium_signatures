...
/**
 * Initialize hash context based on the digest size used by Picnic. If the size
 * is 32 bytes, blake3 is used, otherwise we exit.
 */
static inline void hash_init(hash_context *ctx, size_t digest_size) {
  if (digest_size <= 32) {
...
static inline void hash_update(hash_context *ctx, const uint8_t *data,
                               size_t size) {
  blake3_hasher_update(&ctx->ctx, data, size);
}

static inline void hash_final(hash_context *ctx) { ctx->seek = 0; }

static inline void hash_squeeze(hash_context *ctx, uint8_t *buffer,
                                size_t buflen) {
  blake3_hasher_finalize_seek(&ctx->ctx, ctx->seek, buffer, buflen);
  ctx->seek += buflen;
}
...
static inline void hash_init_x4(hash_context_x4 *ctx, size_t digest_size) {
  if (digest_size <= 32) {
    blake3_hasher_init(&ctx->ctx0);
    blake3_hasher_init(&ctx->ctx1);
    blake3_hasher_init(&ctx->ctx2);
    blake3_hasher_init(&ctx->ctx3);
    ctx->seek = 0;
  } else {
    exit(123);
  }
...
static inline void hash_update_x4(hash_context_x4 *ctx, const uint8_t **data,
                                  size_t size) {
  blake3_hasher_update(&ctx->ctx0, data[0], size);
  blake3_hasher_update(&ctx->ctx1, data[1], size);
  blake3_hasher_update(&ctx->ctx2, data[2], size);
  blake3_hasher_update(&ctx->ctx3, data[3], size);
}
...
static inline void hash_final_x4(hash_context_x4 *ctx) { ctx->seek = 0; }

static inline void hash_squeeze_x4(hash_context_x4 *ctx, uint8_t **buffer,
                                   size_t buflen) {
  blake3_hasher_finalize_seek(&ctx->ctx0, ctx->seek, buffer[0], buflen);
  blake3_hasher_finalize_seek(&ctx->ctx1, ctx->seek, buffer[1], buflen);
  blake3_hasher_finalize_seek(&ctx->ctx2, ctx->seek, buffer[2], buflen);
  blake3_hasher_finalize_seek(&ctx->ctx3, ctx->seek, buffer[3], buflen);
  ctx->seek += buflen;
}
