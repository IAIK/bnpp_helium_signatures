...
// the shuffles bring the aes state into column-first order
// the second shuffle returns to aes-ni order, but also performs shiftrows
// the rotl8 + xors compute the affine transform
// and then party 0 adds the affine constant
...
#define save_s_shares_last(m, s_shares, party, sbox_index)                     \
  do {                                                                         \
    __m128i tmp;                                                               \
    tmp = _mm_shuffle_epi8(m, _mm_set_epi8(15, 11, 7, 3, 14, 10, 6, 2, 13, 9,  \
                                           5, 1, 12, 8, 4, 0));                \
...
#define save_t_shares_last(m, t_shares, party, sbox_index)                     \
  do {                                                                         \
    __m128i tmp;                                                               \
    m = _mm_shuffle_epi8(m, _mm_set_epi8(3, 15, 11, 7, 6, 2, 14, 10, 9, 5, 1,  \
                                         13, 12, 8, 4, 0));                    \
    if (party == 0)                                                            \
      m = _mm_xor_si128(m, _mm_set1_epi8(AES_SBOX_AFFINE_CONST));              \
    tmp = _mm_xor_si128(_mm_rotl_epi8(m, 1), _mm_rotl_epi8(m, 3));             \
    ...
    tmp = _mm_xor_si128(tmp, _mm_rotl_epi8(tmp, 1));                           \
    m = _mm_xor_si128(tmp, m);                                                 \
    _mm_storeu_si128((__m128i *)(&t_shares[party][sbox_index]), m);            \
  } while (0)
...
void aes_128_s_shares(const std::vector<gsl::span<uint8_t>> &key_in,
                      std::vector<gsl::span<uint8_t>> &t_shares,
                      const std::vector<uint8_t> &plaintext_in,
...
#pragma GCC diagnostic pop
  ...
  for (party = 0; party < num_parties; party++) {
    ...
    save_s_shares_last(state, s_shares, party, sbox_index);
    if (party == 0) {
      state = _mm_loadu_si128((__m128i *)ciphertext_in.data());
      state = _mm_xor_si128(state, key_schedule[party][10]);
    } else
      state = key_schedule[party][10];
    save_t_shares_last(state, t_shares, party, sbox_index);
  }
}
