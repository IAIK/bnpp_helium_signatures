...
TEST_CASE("Sign and verify a message", "[signature]") {
  ...
  std::vector<uint8_t> serialized_signature =
      bnpp_sign(instance, keypair, (const uint8_t *)message, strlen(message));
  std::cout << "signature length: " << serialized_signature.size()
            << " bytes\n";
  REQUIRE(bnpp_verify(instance, keypair.second, serialized_signature,
                      (const uint8_t *)message, strlen(message)));
  BENCHMARK("signing") {
    return bnpp_sign(instance, keypair, (const uint8_t *)message,
                     strlen(message));
  };
  BENCHMARK("verification") {
    return bnpp_verify(instance, keypair.second, serialized_signature,
                       (const uint8_t *)message, strlen(message));
  };
}
