...
void expand_seed(const gsl::span<uint8_t> &seed, const salt_t &salt,
...
void expand_seed_x4(const gsl::span<uint8_t> &seed0,
                    ...
                    const gsl::span<uint8_t> &seed3, const salt_t &salt,
                    const size_t rep_idx, const size_t node_idx,
...
SeedTree::SeedTree(const std::vector<uint8_t> &seed, const size_t num_leaves,
                   const salt_t &salt, const size_t rep_idx)
  ...
  for (; i < (last_non_leaf / 4) * 4; i += 4) {
    if (!(node_exists(i + 0) || node_exists(i + 1) || node_exists(i + 2) ||
          node_exists(i + 3)))
      continue;
