...
static int bench_sign_and_verify_free(const bench_options_free_t *options) {
  ...
  case 172:
    ...
    instance.num_lifted_multiplications = 20;
    break;
  case 200:
    instance.block_cipher_params = {16, 16, 200};
    instance.num_lifted_multiplications = 23;
    break;
  }
  ...
  for (unsigned int i = 0; i != options->iter; ++i) {
    ...
    keypair_t keypair = bnpp_keygen(instance);
    ...
    std::vector<uint8_t> signature =
        bnpp_sign(instance, keypair, m, sizeof(m));
    ...
    start_time = timing_read(&ctx);
    bool ok = bnpp_verify(instance, keypair.second, signature, m, sizeof(m));
    tmp_time = timing_read(&ctx);
